...
export function SpireScreen() {
  ...
  // не рендерим эксклюзивную ветку (Консоль ИЛИ бургер) до маунта —
  // иначе на телефоне мелькает десктоп-консоль, а бургер появляется позже
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-bg-0">
      ...
      {/* десктоп/планшет — плавающая консоль; мобайл — нижний бар + шиты.
          Рендерим только после маунта, чтобы не мелькала чужая ветка. */}
      {mounted && !isMobile && <Console />}
      {mounted && isMobile && (
        <>
          <MobileBottomBar />
          <MobileSheets />
      ...
      <AnimatePresence>
        {focusId && (
          <motion.button
            ...
            className="liquid-glass fixed left-1/2 z-[5] flex -translate-x-1/2 items-center gap-2 rounded-full px-4 py-2 font-mono text-[11px] uppercase tracking-wide text-hi"
