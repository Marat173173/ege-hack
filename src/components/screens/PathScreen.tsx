...
const NODE_GAP = 150; // расстояние между узлами по вертикали (с запасом под 2-строчные подписи)
      ...
      >
        ...
        {ordered.map(({ f, i }, renderIdx) => {
          ...
          return (
            <div
              key={f.id}
              className="absolute flex w-[150px] -translate-x-1/2 flex-col items-center"
              style={{ left: `${cx}%`, top }}
            >
              {/* узел (центр круга совпадает с соединительной линией) */}
              <motion.button
                onClick={() => tap(f, i)}
              ...
              {/* подпись темы — ПОД узлом, перенос длинных названий (макс. 2 строки) */}
              <div className="mt-1.5 w-full text-center leading-tight">
                <div className="line-clamp-2 text-[12px] font-semibold leading-tight text-hi">{f.name}</div>
                {locked ? (
                  <div className="mt-0.5 font-mono text-[9px] text-lo">+{unlockGap(floors, i)}% до открытия</div>
                ) : (
                  <div className="mt-0.5 font-mono text-[9px]" style={{ color: meta.color }}>
                    {meta.label.split(" ")[0]}
                  </div>
                )}
              </div>
              ...
              {isStart && (
                <motion.div
                  ...
                  className="mt-1.5 w-fit rounded-lg border border-accent/50 bg-bg-0/80 px-2.5 py-1 font-mono text-[10px] uppercase tracking-wide text-accent backdrop-blur-md"
                >
                  Начать
                </motion.div>
              )}
